...
{
    public class OverllMPIViewModel : ViewModelBase
    {
            ...
            var task = new Task(new Action(async () =>
            {
                ...
                try
                {
                    ...
                    using (var client = new HttpClient(handler))
                    {
                        ...
                        var url = string.Format(APIBase, "mpi", Product, StartDateValue, EndDateValue, true);
                        var response = await client.GetStringAsync(url);
                        // Parse JSON response.
                        var data = JsonConvert.DeserializeObject<ObservableCollection<MPIModel>>(response);
                        foreach (var item in data)
                        {
                            OverallMPIData.Add(new MPIModel() { Labor = item.Labor, RepliedVol = item.RepliedVol, ReplyMPI = item.ReplyMPI });
                        }
                    }
                }
                catch (Exception ex)
